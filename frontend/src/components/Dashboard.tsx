    ...
    <>
      <section className="mb-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {cards.map(([label, value, detail, color]) => (
          ...
          >
            <div className={`mb-5 size-10 rounded-2xl ${color}`} />

            <p className="text-sm text-stone-500">{label}</p>

            <p className="my-1 font-['Manrope'] text-3xl font-extrabold">
              {value}
            </p>

            <p className="text-xs font-medium text-stone-400">{detail}</p>
          </article>
        ))}
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
            ...
            {orders.slice(0, 5).map((order) => (
              ...
              >
                ...
                <span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-bold text-amber-700">
                  {t(`status_${order.status}` as TranslationKey)}
                </span>
              </div>
            ))}
        ...
        <article className="rounded-3xl bg-ink p-6 text-white shadow-card">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="mt-8 inline-block rounded-xl bg-lime px-4 py-3 text-sm font-bold text-ink"
          >
            {t('api_docs')}
          </a>
        </article>
